...
class QueueManager {
public:
    QueueManager():pUteMsgMpmcQueue_{nullptr},pUteMsgMpmcShareSlots_{nullptr},
    ...
    /// @return 
    bool Init(StrategyGetRspCallbackMessageFuncType StraegyOnMessageFunc,
    ...
    /// @param iMsgLen : 消息长度;
    /// @param ulStrategyKey : 策略进程的唯一标识 - 策略端写入请求时需要填入， UTE端写回报不需要写；
    void SendMsgShare(int iMsgID, const char* pMsgBuf, const int iMsgLen, unsigned long long ulStrategyKey =0);    
...
private:
    mpmc_queue<UteMsg>*    pUteMsgMpmcQueue_;         // 无锁队列;
    element_slot<UteMsg, false>*  pUteMsgMpmcShareSlots_;  // 无锁队列存储真正数据元素的起始地址 -- 用于共享内存映射无锁队列时使用；
    ...
    StrategyGetRspCallbackMessageFuncType StraegyOnMessageFunc_;
...
/// @brief 策略进程发送非委托请求给UTE的共享内存;
/// @param cstrFileKey 
/// @return 
inline std::string GetNonReqOrderQueueName(const char* cstrFileKey) {
    return std::string("Strategy_UTE_NonReqOrder") + std::string(cstrFileKey) + ".queue";
}

/// @brief 策略进程发送非委托请求给UTE的共享内存;
/// @param cstrFileKey 
/// @return 
inline std::string GetReqOrderQueueName(const char* cstrFileKey) {
    return std::string("Strategy_UTE_ReqOrder") + std::string(cstrFileKey) + ".queue";
}
