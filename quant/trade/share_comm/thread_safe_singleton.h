...
class ThreadSafeSingleton
{
    public:
        ...
        static std::shared_ptr<T>& DoubleCheckInstance(Args&&... args)
        {
            if (nullptr == m_unique_instance)
            {
                ...
                if (nullptr == m_unique_instance)
                {
                    ...
                    m_unique_instance = std::make_shared<T>(std::forward<Args>(args)...);
                }
            }
...
private:
    ...
    static std::shared_ptr<T>   m_unique_instance;
...
template<class T> std::shared_ptr<T> ThreadSafeSingleton<T>::m_unique_instance = nullptr;
