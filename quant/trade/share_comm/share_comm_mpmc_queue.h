...
public:
    ...
    bool create_shared(uint32_t size, slot_type* & pMpmcShareSlots, void* buffer)
    {
        ...
        for (uint32_t i = 0; i < size; ++i)
        {
            pMpmcShareSlots[i].init(UINT32_MAX - 1); // 初始化所有槽的turn值
        }
        #endif
    ...
    bool slot_attach(slot_type* & pMpmcShareSlots, void* buffer) {
        pMpmcShareSlots = static_cast<slot_type*>(buffer);
        return true;
    }
    ...
    bool trypush(Args&&... args)
    {
        ...
        LOG_DEBUG("cur_turn: {}, index: {}, ticket: {}", cur_turn, index, ticket);
        // 检查元素槽是否可入队
    ...
    /**
    * @brief: 共享内存版本的，trypush， pSlot 指针由外部传入;
    */
    template <class... Args>
    bool trypush_share(slot_type* pSlot, Args&&... args)
    {
        // 获取当前生产者ticket
        uint64_t ticket = push_ticket_.load(std::memory_order_acquire);
        auto index = idx(ticket);
        ...
        LOG_DEBUG("cur_turn: {}, index: {}, ticket: {}", cur_turn, index, ticket);
        // 检查元素槽是否可入队
        if (pSlot[index].may_enqueue(cur_turn))
        {
            // CAS操作尝试更新ticket，成功则执行入队
            if (push_ticket_.compare_exchange_strong(ticket, ticket + 1))
            {
                pSlot[index].pre_validated_enqueue(std::forward<Args>(args)...);
                return true;
            }
        }
        return false;
     ...
     * @note 非阻塞版本，使用CAS操作尝试获取出队权限
     */
    bool TryPopShare(slot_type* pSlot, T& element)
    {
        // 获取当前消费者ticket
        uint64_t ticket = pop_ticket_.load(std::memory_order_acquire);
        auto index = idx(ticket);
        ...
        // LOG_DEBUG("cur_turn: {}, index: {}, ticket: {}", cur_turn, index, ticket);
        // 检查元素槽是否可出队
        if (pSlot[index].may_dequeue(cur_turn))
        {
            // CAS操作尝试更新ticket，成功则执行出队
            if (pop_ticket_.compare_exchange_strong(ticket, ticket + 1))
            {
                pSlot[index].pre_validated_dequeue(element);
                return true;
            }
        }
        return false;
     ...
     * @note 阻塞版本，会一直等待直到出队成功
     */
    int pop_share(slot_type* pSlot, T& element)
    {
        // 获取并自增消费者ticket
        uint64_t ticket = pop_ticket_++;
        auto index = idx(ticket);
        auto cur_turn = turn(ticket); 
        // 执行出队操作，可能阻塞
        pSlot[index].dequeue(cur_turn, element);
        return index;
    }    
...
element_slot(int iTestIndex):current_turn_(0){
    iSlotInnerTestIndex_ = iTestIndex;
}
