...
bool MarketOutput::InitShareMarketDataQueue() {
    ...
    unsigned int uiElementSlotBlocksSize = (roundup_pow_of_two(iQueueSize_)) * sizeof( element_slot<KlineAtom, false>) ;

    // 设置共享内存大小
    unsigned int uiMemorySize_ = sizeof(mpmc_queue<KlineAtom>) + uiElementSlotBlocksSize + 128; // 给共享内存留足够的空间;
    cout << "uiMemorySize_: " << uiMemorySize_ << endl;
    ...
    ptr_share_market_data_queue_ = new (addr) mpmc_queue<KlineAtom>();
    if (!ptr_share_market_data_queue_) {
        LOG_ERROR("new ptr_share_market_data_queue_ Failed!");
        return false;
    }   

    element_slot<KlineAtom, false>* pSlotAddr = (element_slot<KlineAtom, false>*)((char*)ptr_share_market_data_queue_slot_ 
                                                    + uiMemorySize_ - sizeof(element_slot<KlineAtom, false>));
    printf("pSlotAddr: %p\n", pSlotAddr);
