...
public:
    LockFileManager():iHeartBeatSec_{1}, shptrHearbeatThread_{nullptr}, 
        WriteOnEventFunc_{nullptr}, ReadOnEventFunc_{nullptr},
        iWaitWriteSec_{1},shptrWaitWriteLockFileThread_{nullptr},bIsRunning_{true} {}
        ...
        ~LockFileManager() {
            ...
            if (shptrWaitWriteLockFileThread_ && shptrWaitWriteLockFileThread_->joinable()) {
                shptrWaitWriteLockFileThread_->join();
            }
    ...
    /// @brief 策略进程端初始化接口;
    /// 1、判断 cstrUtWriteme 对应的锁文件是否存在并且加锁，若是失败，则循环式的继续判断；
    /// 2、若是上面判断成功，创建策略进程对应的锁文件，并且加锁；
    /// 3、之后便开启心跳线程，每隔一段时间，检查UTE锁文件是否存在并加锁;
    /// @param cstrWriteName UTE进程名，也是锁文件名；
    /// @param cstrReadLockFile 策略进程名，也是锁文件名；
    /// @param pReadMessageManager 策略进程的消息管理器；
    /// @param iEventSleepSec 事件监听线程的睡眠时间间隔，默认为5秒；
    /// @return 是否成功初始化
    bool Init(const char* cstrWriteName, unsigned long long ulReadKey, 
              ReadGetRspCallbackEventFuncType ReadOnEventFunc,
              long lStartSec,  int iEventSleepSec = 3, int iWaitWriteSec = 10);
    ...
    /// @param iEventSleepSec 事件监听线程的睡眠时间间隔，默认为5秒；
    bool Init(const char* cstrWriteName,  WriteGetReadReqCallBackFuncType WriteOnEventFunc, int iEventSleepSec = 5);              

    // 通过本地以策略ID为名的文件，获取对应的上次的BatchID；
    unsigned int GetSetReadBatchID(unsigned int ReadSysID);
...
private:
    ...
    WriteGetReadReqCallBackFuncType WriteOnEventFunc_;  // UTE进程 OnEvent 的回调接口, 用于通知UTE 某个 策略进程终止的消息;
    ReadGetRspCallbackEventFuncType ReadOnEventFunc_; // Read进程 OnEvent 的回调接口, 用于通知策略进程 UTE终止的消息;


    int  iWaitWriteSec_;      // 等待UTE启动时间间隔 - 策略进程专用;    

    std::shared_ptr<std::thread> shptrWaitWriteLockFileThread_;
